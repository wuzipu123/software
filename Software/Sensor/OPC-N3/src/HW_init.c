 ...
 */
void HW_init_SPI(SPI_HandleTypeDef *SpiHandle)
 {
	...
	SpiHandle->Init.NSS               = SPI_NSS_SOFT; //uC is master, one participant on nss
...
void HW_init_UART(UART_HandleTypeDef *UartHandle)
{
	...
	/* PA3 UART1_RX only used for full duplex communication
	 * PA4 UART1_TX
	 * PC14 UART1_RE
	 * PC15 UART1_DE
	 */
	...
	GPIO_Init_UART.Alternate = GPIO_AF6_LPUART1;
	HAL_GPIO_Init(GPIOA, &GPIO_Init_UART);

	//PA4 configured as TX
	GPIO_Init_UART.Pin 			= GPIO_PIN_4;
	GPIO_Init_UART.Mode      	= GPIO_MODE_AF_PP;
	GPIO_Init_UART.Pull			= GPIO_PULLUP;
	GPIO_Init_UART.Speed     	= GPIO_SPEED_FREQ_VERY_HIGH;
	GPIO_Init_UART.Alternate 	= GPIO_AF6_LPUART1;
 ...
 */
//FIXME .weak is not working in the startup file for LPUART1 ISR
void LPUART1_IRQHandler_ManOverride (void)
{
